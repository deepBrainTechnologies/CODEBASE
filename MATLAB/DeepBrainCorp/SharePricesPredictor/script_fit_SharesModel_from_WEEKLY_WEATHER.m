...
for ii=1:1:(length(mondaysList)-1)
  ...
  for dd=1:1:length(DeltaTemp)
    ...
    subplot(2,2,ii);
    scatter(dd,thisNvidia(dd)+AAofminerror(ii,dd)*DeltaTemp(dd),'*k');hold all;
    scatter(dd,thisNvidia(dd)+meanAA(ii,dd)*DeltaTemp(dd),'rs'); hold all;
    scatter(dd,thisNvidia(dd)+(meanAA(ii,dd)-stdAA(ii,dd))*DeltaTemp(dd),'rs'); hold all;
    scatter(dd,thisNvidia(dd)+(meanAA(ii,dd)+stdAA(ii,dd))*DeltaTemp(dd),'rs'); hold all;
    scatter(dd,thisNvidia(dd+1),'db');
  end
