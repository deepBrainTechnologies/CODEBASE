  ...
  for folder = 3:1:nFolders  %X11_INPUT SUBFOLDERS
    ...
    for delComb=1:1:nDelayComb   %parfor delComb=1:nDelayComb
      for gainComb=1:1:nGainComb
        ...
        [PDF_Xn2iJoint_Unif, Entropy_Xn2iJoint_Unif] =classCalculatePDFbasedEntropy.H_ofJointBinarySignal_1BitLongWord(rasterUnif);
        
        [rasterExpo, rasterSamples] = tool_sampleIX_to_raster(outSpksSampIX_EXPON,outSpks_Amp_EXPON,nStepsToSimPerCase);
        [PDF_Xn2iJoint_Expon, Entropy_Xn2iJoint_Expon] =classCalculatePDFbasedEntropy.H_ofJointBinarySignal_1BitLongWord(rasterExpo);
        
        %%[rasterGauss, rasterSamples] = tool_sampleIX_to_raster(outSpksSampIX_GAUSS,outSpks_Amp_GAUSS,nStepsToSimPerCase);
        %%[PDF_Xn2iJoint_Gauss, Entropy_Xn2iJoint_Gauss] =classCalculatePDFbasedEntropy.H_ofJointBinarySignal_1BitLongWord(rasterGauss);
        ...
        save(filename,'PDF_Xn2iJoint_Unif','Entropy_Xn2iJoint_Unif','rasterUnif',...  
                     ...
                     % 'PDF_Xn2iJoint_Gauss','Entropy_Xn2iJoint_Gauss','rasterGauss');  
        ...
        %%%Entropy_X11_combination = mean(X11_results.Entropy_x11,1); %PDF based averaged over trials
...
%        Entropy_ALL_Gauss = cat(1,Entropy_ALL_Gauss,Entropy_Xn2iJoint_Gauss);
  ...
  %%filename=[Xn2iFIGFOLDER '/figure_X11GAUSSdelayDist__XN2i_Joint_PDFbased_Entropy3lines_1bitword.png'];
  %%classPlotScatter.plotTrials(filename,Entropy_ALL_Gauss(:),Entropy_X11_ALL(:),[]);
