...
#define Kd _p[15]
#define Kd_columnindex 15
#define B0 _p[16]
#define B0_columnindex 16
#define Dca (_p + 17)
#define Dca_columnindex 17
#define DCaBuffer (_p + 21)
#define DCaBuffer_columnindex 21
#define DBuffer (_p + 25)
#define DBuffer_columnindex 25
#define _g _p[29]
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "diam");
 	...
 	ivoc_help("help ?1 cadifus cadif.mod\n");
...
for (_i=0; _i < 4; _i++) {
  	_RHS1(_i + 4) *= ( diam * diam * vol [ ((int) _i ) ]) ;
_MATELM1(_i + 4, _i + 4) *= ( diam * diam * vol [ ((int) _i ) ]);  } 
for (_i=0; _i < 4; _i++) {
  	_RHS1(_i + 8) *= ( diam * diam * vol [ ((int) _i ) ]) ;
_MATELM1(_i + 8, _i + 8) *= ( diam * diam * vol [ ((int) _i ) ]);  } }
 ...
 */
 ...
 for (_i=0; _i < 4; _i++) { _p[_dlist1[_i + 4]] /= ( diam * diam * vol [ ((int) _i ) ]);}
 for (_i=0; _i < 4; _i++) { _p[_dlist1[_i + 8]] /= ( diam * diam * vol [ ((int) _i ) ]);}
   } return _reset;
 }
 ...
 v=_v;
{
 ...
 if(error){fprintf(stderr,"at line 65 in file cadif.mod:\n	SOLVE state METHOD sparse\n"); nrn_complain(_p); abort_run(error);}
    if (secondorder) {
    int _i;
    for (_i = 0; _i < 12; ++_i) {
...
#if NMODL_TEXT
static const char* nmodl_filename = "cadif.mod";
static const char* nmodl_file_text = 
  "\n"
